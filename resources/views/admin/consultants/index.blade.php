...
@section('title', __('consultants.title'))
@section('content')
        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <!--begin::Title-->
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">{{ __('consultants.list') }}</h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                        ...
                        <!--end::Item-->
                        <!--begin::Item-->
                        <li class="breadcrumb-item text-muted">{{ __('consultants.title') }}</li>
                        <!--end::Item-->
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
                    ...
                    <div class="card-header border-0 pt-6">
                        ...
                        <div class="card-title">
                            ...
                            <div class="d-flex align-items-center position-relative my-1">
                                ...
                                <input type="text" data-kt-user-table-filter="search"
                                       class="form-control form-control-solid w-250px ps-14" placeholder="{{ __('consultants.search') }}"/>
                            </div>
                            <!--end::Search-->
                        </div>
                        <!--begin::Card title-->
                        ...
                        <div class="card-toolbar">
                            ...
                            <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::CREATE_CONSULTANTS->value))
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                            data-bs-target="#kt_modal_add_consultant">
                                        <span class="svg-icon svg-icon-2"><i class="fa-solid fa-plus"></i></span>
                                        {{ __('consultants.create') }}
                                    </button>
                                @endif
                            </div>
                            <!--end::Toolbar-->
                    ...
                    <div class="card-body py-4">
                        ...
                        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
                            ...
                            <tr class="text-gray-400 fw-bold fs-7 text-uppercase gs-0 text-center">
                                <th class="min-w-125px">{{ __('consultants.name') }}</th>
                                <th class="min-w-125px">{{ __('consultants.bio') }}</th>
                                <th class="min-w-125px">{{ __('consultants.email') }}</th>
                                <th class="min-w-125px">{{ __('consultants.phone') }}</th>
                                <th class="min-w-100px">{{ __('consultants.actions') }}</th>
                            </tr>
                            <!--end::Table row-->
                            </thead>
                            <!--end::Table head-->
    ...
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::CREATE_CONSULTANTS->value))
        ...
        @include('admin.consultants.modals.create')
        <!--end::Modal - Add Users-->
    @endif
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::UPDATE_CONSULTANTS->value))
        <!--begin::Modal - Update user-->
        @include('admin.consultants.modals.edit')
        <!--end::Modal - Update user-->
    @endif
@endsection
@section('scripts')
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::LIST_CONSULTANTS->value))
        <script>
    ...
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::CREATE_CONSULTANTS->value))
        <script>
            ...
            var KTUsersAddUser = function () {
                // Shared variables
                const element = document.getElementById('kt_modal_add_consultant');
                const form = element.querySelector('#kt_modal_add_consultant_form');
    ...
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::UPDATE_CONSULTANTS->value))
        <script>
            ...
            var KTUsersUpdateDetails = function () {
                // Shared variables
                const element = document.getElementById('kt_modal_update_consultant');
                const form = element.querySelector('#kt_modal_update_consultant_form');
                const modal = new bootstrap.Modal(element);
                        ...
                        if (validator) {
                            validator.validate().then(function (status) {
                                if (status === 'Valid') {
                                    ...
                                    let formData = new FormData(form);
                                    let userId = $('#kt_modal_update_consultant_form').data('user-id');
                                    let updateUrl = `/consultants/${userId}`;
                                    $.ajax({
                                        url: updateUrl,
                                        type: "POST",
                                        data: formData,
    ...
    @if(auth()->user()->hasPermissionTo(\App\Enums\PermissionEnum::DELETE_CONSULTANTS->value))
        <script>
            // handle delete user
            $(document).on("click", '[data-kt-users-table-filter="delete_row"]', function (e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.consultants.destroy', ':id') }}".replace(':id', userId),
                            type: 'DELETE',
                            ...
                            error: function () {
                                Swal.fire({
                                    text: "Failed to delete the Consultant. Try again Later!",
                                    icon: "error",
                                    buttonsStyling: false,
                                    confirmButtonText: "Ok",
                                    customClass: {
    ...
    <script>
    // Helper to populate school select options
    function populateSchoolSelect(selectId, schools, selectedIds = []) {
        const select = $(selectId);
        select.empty();
        schools.forEach(school => {
            const option = $('<option></option>')
                .val(school.id)
                .text(school.user ? school.user.name : 'School #' + school.id);
            if (selectedIds.includes(school.id)) {
    ...
    // CREATE MODAL: Populate schools on open
    $('#kt_modal_add_consultant').on('show.bs.modal', function () {
        $.ajax({
            url: '/consultants/create',
            type: 'GET',
            success: function (response) {
                // Assume response is an array of schools with user relation
                populateSchoolSelect('#create_school_ids', response.schools || response);
            },
            error: function () {
    ...
    $(function() {
        $('#create_school_ids').select2({
            dropdownParent: $('#kt_modal_add_consultant')
        });
        $('#edit_school_ids').select2({
            dropdownParent: $('#kt_modal_update_consultant')
        });
